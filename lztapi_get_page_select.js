var pageId = GetInputConstructorValue('pageId', loader)
if (pageId['original'].length == 0) {
	Invalid(tr('The parameter "') + tr('pageId') + tr('" is not specified'))
	return
}
...
try {
	var code =
		...
		_.template($('#lztapi_get_page_code').html())({
			pageId: pageId['updated'],
