var tagId = GetInputConstructorValue('tagId', loader)
if (tagId['original'].length == 0) {
	Invalid(tr('The parameter "') + tr('tagId') + tr('" is not specified'))
	return
}
...
try {
	var code =
		...
		_.template($('#lztapi_get_tagged_content_code').html())({
			tagId: tagId['updated'],
            page: page['updated'],
            limit: limit['updated'],
