var notificationId = GetInputConstructorValue('notificationId', loader)
if (notificationId['original'].length == 0) {
	Invalid(tr('The parameter "') + tr('notificationId') + tr('" is not specified'))
	return
}
...
try {
	var code =
		...
		_.template($('#lztapi_get_notification_code').html())({
			notificationId: notificationId['updated'],
