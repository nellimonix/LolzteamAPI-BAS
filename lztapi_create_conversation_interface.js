<div class="container-fluid">
    <%= _.template($('#checkbox').html())({
        id: "is_group",
        title: tr("Is group. Set <code>disabled</code> if personal conversation, or set <code>enabled</code> if group"),
        checked: false
    }) %>
	<%= _.template($('#input_constructor').html())({
		id: "recipient_id",
		description: tr("recipient_id"),
        visible_if_unchecked: "is_group",
		default_selector: "int",
		disable_string: true,
		value_number: "",
		help: {
			description: tr("Id of recipient."),
		}
	}) %>
    <%= _.template($('#input_constructor').html())({
        id: "recipients",
        description: tr("recipients"),
        visible_if_checked: "is_group",
        default_selector: "string",
        disable_int: true,
        value_string: "",
        help: {
            description: tr("List of recipients username's. Max recipients count is 10 (Separated by comma)."),
            examples: [
                {code: "llimonix"},
                {code: "vuchaev2015"},
                {code: "весть,hashbrute", description: tr("Specify multiple username's.") + " " +  tr("Must be separated by <code>,</code>.")}
            ]
        }
    }) %>
    <%= _.template($('#input_constructor').html())({
        ...
        value_string: "",
        help: {
            description: tr("The title of new conversation."),
        }
    }) %>
	<%= _.template($('#block_start').html())({
    	...
    	<%= _.template($('#input_constructor').html())({
            ...
            textarea_height: 80,
            help: {
                description: tr("Message. Required if <code>is_group = disabled</code>")
            }
        }) %>
		<%= _.template($('#checkbox').html())({
			id: "open_invite",
			title: tr("Open invite"),
			checked: false
		}) %>
        <%= _.template($('#checkbox').html())({
			id: "conversation_locked",
			title: tr("Is conversation locked"),
			checked: false
		}) %>
        <%= _.template($('#checkbox').html())({
			id: "allow_edit_messages",
			title: tr("Allow edit messages"),
			checked: false
		}) %>
...
<div class="tooltipinternal">
	<div class="tr tooltip-paragraph-first-fold">Create a new conversation.</div>
</div>
