var room_id = GetInputConstructorValue('room_id', loader)
if (room_id['original'].length == 0) {
	Invalid(tr('The parameter "') + tr('room_id') + tr('" is not specified'))
	return
}
...
try {
	var code =
		...
		_.template($('#lztapi_get_chat_online_code').html())({
