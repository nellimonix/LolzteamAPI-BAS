var searchId = GetInputConstructorValue('searchId', loader)
if (searchId['original'].length == 0) {
	Invalid(tr('The parameter "') + tr('searchId') + tr('" is not specified'))
	return
}
...
try {
	var code =
		...
		_.template($('#lztapi_get_search_results_code').html())({
			searchId: searchId['updated'],
			page: page['updated'],
			limit: limit['updated'],
