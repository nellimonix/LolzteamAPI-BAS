var link = GetInputConstructorValue('link', loader)
if (link['original'].length == 0) {
	Invalid(tr('The parameter "') + tr('link') + tr('" is not specified'))
	return
}
...
try {
	var code =
		...
		_.template($('#lztapi_save_conversation_code').html())({
			link: link['updated'],
