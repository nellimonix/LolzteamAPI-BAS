var messageId = GetInputConstructorValue('messageId', loader)
if (messageId['original'].length == 0) {
	Invalid(tr('The parameter "') + tr('messageId') + tr('" is not specified'))
	return
}

var message_body = GetInputConstructorValue('message_body', loader)
if (message_body['original'].length == 0) {
...
try {
	var code =
		...
		_.template($('#lztapi_edit_conversation_message_code').html())({
