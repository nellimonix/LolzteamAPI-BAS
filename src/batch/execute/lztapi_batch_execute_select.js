var jobs = GetInputConstructorValue('jobs', loader)
if (jobs['original'].length == 0) {
	Invalid(tr('The parameter "') + tr('jobs') + tr('" is not specified'))
	return
}
...
try {
	var code =
		...
		_.template($('#lztapi_batch_execute_code').html())({
			jobs: jobs['updated'],
