...
var commentId = GetInputConstructorValue('commentId', loader)
if (commentId['original'].length == 0) {
	Invalid(tr('The parameter "') + tr('commentId') + tr('" is not specified'))
	return
}
...
try {
	var code =
		...
		_.template($('#lztapi_get_profile_post_comment_code').html())({
            profilePostId: profilePostId['updated'],
			commentId: commentId['updated'],
