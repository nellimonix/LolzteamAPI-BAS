var categoryId = GetInputConstructorValue('categoryId', loader)
if (categoryId['original'].length == 0) {
	Invalid(tr('The parameter "') + tr('categoryId') + tr('" is not specified'))
	return
}
...
try {
	var code =
		...
		_.template($('#lztapi_get_category_code').html())({
			categoryId: categoryId['updated'],
