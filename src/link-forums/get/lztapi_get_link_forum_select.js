var linkId = GetInputConstructorValue('linkId', loader)
if (linkId['original'].length == 0) {
	Invalid(tr('The parameter "') + tr('linkId') + tr('" is not specified'))
	return
}
...
try {
	var code =
		...
		_.template($('#lztapi_get_link_forum_code').html())({
			linkId: linkId['updated'],
