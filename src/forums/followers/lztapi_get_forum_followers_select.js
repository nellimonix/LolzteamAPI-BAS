...
if (forumId['original'].length == 0) {
	Invalid(tr('The parameter "') + tr('forumId') + tr('" is not specified'))
	return
}
...
try {
	var code =
		...
		_.template($('#lztapi_get_forum_followers_code').html())({
			forumId: forumId['updated'],
