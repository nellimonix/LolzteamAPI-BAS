<div class="container-fluid">
	...
	<%= _.template($('#block_start').html())({
        ...
        <%= _.template($('#input_constructor').html())({
            id: "response_id",
            description: tr("response_id"),
            default_selector: "int",
            disable_string: true,
            value_number: "",
            help: {
                description: tr("The id of the response to vote for. Can be skipped if <code>response_ids</code> set.")
            }
        }) %>
		<%= _.template($('#input_constructor').html())({
			id: "response_ids",
			description: tr("response_ids"),
			default_selector: "string",
			disable_int: true,
			value_string: "",
			help: {
				description: tr("List of ids of responses (if the poll allows multiple choices)."),
				examples: [
					{code: "1"},
					{code: "2"},
					{code: "1,2", description: tr("Specify multiple response IDs.") + " " +  tr("Must be separated by <code>,</code>.")}
				]
			}
		}) %>
...
<div class="tooltipinternal">
	<div class="tr tooltip-paragraph-first-fold">Vote on a thread poll.</div>
</div>
